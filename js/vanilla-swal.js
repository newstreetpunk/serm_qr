/*
	...
	Заменяет одну известную библиотеку в нашем проекте, сохраняя совместимость по API там,
	где это нужно (fire, close, showValidationMessage) и классы, которые уже используются в коде.

	Почему так:
	- Отказались от зависимости от одной известной библиотеки, но текущий код опирается на вызовы Swal.fire/close и
